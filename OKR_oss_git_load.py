...
("oss_git_views", file_path + "oss_git_views.csv", {'view_timestamp'}, 'None', 's', {'view_count', 'view_unique'}),
...
def load_new_table_mapd(connection, table_name, csv_file, ts_cols, ts_format, ts_units, int_cols, mapd_host, mapd_user):
...
def main():
    ...
    if connection != 'RETRY':
        for table, file, timestamp_cols, timestamp_format, timestamp_units, integer_cols in tables_and_files:
            load_new_table_mapd(connection, table, file, timestamp_cols, timestamp_format, timestamp_units, integer_cols, mapdhost, mapduser)
        # disconnect MapD
